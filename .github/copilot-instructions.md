...
7. Supabase
   Creación de clientes: Usar createClient de src\lib\supabase\client.ts para operaciones del lado del cliente y createSupabaseServerClient de src\lib\supabase\server.ts para el lado del servidor/acciones.
...
11. Testing
    Pruebas unitarias: Escribir pruebas unitarias para funciones críticas y componentes reutilizables.

12. Tanstack Query
    Uso de React Query: Utilizar React Query para la gestión del estado del servidor y el almacenamiento en caché de datos asíncronos.
    Ejemplo: useProfile, useUserSubscription
    Mutaciones: Utilizar mutaciones de React Query para operaciones que modifican datos, asegurando la invalidación adecuada de las consultas.
    Ejemplo: useMutation para actualizar el perfil del usuario o gestionar suscripciones.
    Stale Time y Cache Time: Configurar staleTime y cacheTime según la naturaleza de los datos para optimizar el rendimiento y la experiencia del usuario.
    Ejemplo: Datos de perfil del usuario pueden tener un staleTime más largo, mientras que los datos en tiempo real pueden tener un staleTime más corto.
    Manejo de errores: Implementar un manejo de errores robusto utilizando las opciones onError y onSuccess de React Query para proporcionar retroalimentación al usuario.
    Ejemplo: Mostrar un toast de error si una mutación falla.
