...
- **Type Integrity**: Verify that all database interactions are strictly typed using `database.types.ts`. Ensure Server Actions validate inputs with Zod schemas and maintain proper TypeScrip`t interfaces (avoid `any`).
