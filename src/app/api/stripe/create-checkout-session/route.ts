...
export async function POST(req: Request) {
  try {
    ...
    if (!user) {
      console.error(
        "Error: Usuario no autenticado al crear sesión de checkout."
      );
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url:
        cancelUrl ||
        `${process.env.NEXT_PUBLIC_BASE_URL}/plans?payment_canceled=true`,
    });
    ...
    return NextResponse.json(
      { error: `Error al crear la sesión de pago ${err}` },
      { status: 500 }
    );
  }
}
