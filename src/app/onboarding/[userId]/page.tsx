          ...
          <form onSubmit={form.handleSubmit(handleFinalSubmit)}>
            <CardContent className="space-y-6">
              ...
              <Accordion
                ...
                onValueChange={(value) => {
                  if (value === 'step-2-details' && !form.getValues('opposition_id')) {
                    form.setError('opposition_id', {
                      type: 'manual',
                      message: 'Primero debes seleccionar una oposición.',
                    });
                    setActiveAccordionItem('step-1-opposition');
                  } else if (value === 'step-3-study-days' && !form.getValues('objectives')) {
                    setActiveAccordionItem('step-2-details');
                    toast({
                      title: 'Error',
                      description: 'Completa tus preferencias de estudio primero.',
                      variant: 'destructive',
                    });
                  } else {
              ...
              >
                <AccordionItem value="step-1-opposition">
                  <AccordionTrigger>Paso 1: Selecciona tu Oposición Principal</AccordionTrigger>
                  <AccordionContent className="pt-4 space-y-4">
                    {isLoadingOppositions ? (
                      ...
                      <p className="text-muted-foreground">
                        No hay oposiciones disponibles en este momento. Contacta con el
                        administrador.
                      </p>
                    )}
                    <Button
                      ...
                      disabled={
                        !form.watch('opposition_id') || isLoadingOppositions || isSubmittingStep1
                      }
                      className="w-full sm:w-auto"
                    >
                      {isSubmittingStep1 ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                ...
                <AccordionItem value="step-2-details">
                  ...
                  <AccordionContent className="pt-4 space-y-6">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>¿En qué áreas podríamos ayudarte más?</FormLabel>
                          <FormControl>
                    ...
                    <Button
                      ...
                      disabled={!form.watch('objectives') || isSubmittingStep2}
                      className="w-full sm:w-auto"
                    >
                      {isSubmittingStep2 ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                      Siguiente Paso <ChevronRight className="ml-2 h-4 w-4" />
                    </Button>
                  </AccordionContent>
                </AccordionItem>
                    ...
                    <FormField
                      ...
                      render={() => (
                        <FormItem className="space-y-6">
                          <div className="mb-4">
                            <FormLabel className="text-base">Horario de Estudio Semanal</FormLabel>
                            <FormDescription>
                              Configura tus horarios preferidos de estudio para cada día.
                            </FormDescription>
                          </div>
                          <div>
                          ...
                          {form.formState.errors.study_days?.root && (
                            <p className="text-sm font-medium text-destructive">
                              {form.formState.errors.study_days.root?.message}
                            </p>
                          )}
                          <FormMessage />
                        </FormItem>
            ...
            <CardFooter>
              ...
              >
                {isServerActionPending ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Guardando Onboarding...
                  </>
                ) : (
                  <>
                    <Save className="mr-2 h-5 w-5" /> Guardar Onboarding y finalizar
                  </>
                )}
              </Button>
            </CardFooter>
