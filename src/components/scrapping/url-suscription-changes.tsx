...
const fetchUnifiedHistory = async (): Promise<UnifiedChangeEntry[]> => {
...
export function UnifiedChangeHistory({ showLink = false, length = 4 }: UnifiedChangeHistoryProps) {
  const {
    data: history = [],
    isLoading,
    isError,
            ...
            >
              ...
              <div className="p-3 mt-2 bg-secondary/50 rounded-lg border">
                <ul className="list-disc pl-5 space-y-1 text-sm">
                  {(entry.summary.length > 2 ? entry.summary.slice(0, 2) : entry.summary).map(
                    (change, changeIndex) => (
                ...
                {entry.summary.length > 3 && (
                  <Button
                    ...
                    onClick={(e) => {
                      ...
                      if (list) {
                        list.innerHTML = entry.summary
                          .map((change) => `<li class="text-muted-foreground">${change}</li>`)
                          .join('');
                      }
